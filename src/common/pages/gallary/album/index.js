...
export default class Gallery extends React.Component {
  ...
  openGallery = (item) => {
    ...
    this.gallery.listen('gettingData', (index, value) => {
      const that = this
      if (value.w < 1 || value.h < 1) { // unknown size
        const img = new Image()
  ...
  render() {
    ...
    const imgsTag = imgs.map((v1, index1) => (
      v1.map((v2, index2) => (
        <div key={index1 * index2 + Math.random()} className="cloud-box">
          <Card key={index1 * index2 + Math.random()} bordered={true} bodyStyle={{ padding: 0 }}>
            <div>
              <img onClick={() => this.openGallery(v2)} alt="example" width="100%" src={v2} />
            </div>
            <div className="pa-m">
              <h3>React Admin</h3>
              <small><a href="https://github.com/MuYunyun" target="_blank" rel='noopener noreferrer'>https://github.com/MuYunyun</a></small>
            </div>
          </Card>
        </div>
      ))
